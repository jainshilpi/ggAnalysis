...
process.source = cms.Source("PoolSource",
    fileNames = cms.untracked.vstring('file:2018D_DatTest.root')
)
...
process.ecalRecalibRecHit = cms.EDProducer("EcalRecalibRecHitProducer",
    EBRecHitCollection = cms.InputTag("reducedEgamma","reducedEBRecHits"),
    EBRecalibRecHitCollection = cms.string('recalibEcalRecHitsEB'),
    EERecHitCollection = cms.InputTag("reducedEgamma","reducedEERecHits"),
    EERecalibRecHitCollection = cms.string('recalibEcalRecHitsEE'),
...
process.ggNtuplizer = cms.EDAnalyzer("ggNtuplizer",
    ...
    ebReducedRecHitCollection = cms.InputTag("ecalRecalibRecHit","recalibEcalRecHitsEB"),
    ...
    eeReducedRecHitCollection = cms.InputTag("ecalRecalibRecHit","recalibEcalRecHitsEE"),
    ...
    isoTrkSrc = cms.InputTag("isolatedTracks","","RECO"),
    ...
    triggerEvent = cms.InputTag("slimmedPatTrigger","","RECO"),
    triggerResults = cms.InputTag("TriggerResults","","HLT"),
    year = cms.int32(2018)
)
...
process.TFileService = cms.Service("TFileService",
    fileName = cms.string('anTGCtree_data.root')
)
...
process.patMetModuleSequencePuppiUpdated = cms.Sequence(process.pfMetPuppiUpdated+process.ak4PFJetsPuppiUpdated+process.patJetCorrFactorsPuppiUpdated+process.patJetsPuppiUpdated+process.basicJetsForMetPuppiUpdated+process.jetSelectorForMetPuppiUpdated+process.cleanedPatJetsPuppiUpdated+process.metrawCaloPuppiUpdated+process.pfCHS+process.pfMetCHS+process.patCHSMet+process.pfTrk+process.pfMetTrk+process.patTrkMet+process.patPFMetPuppiUpdated)
...
process.ak4JetSubs = cms.Sequence(process.puppi+process.ak4PFJetsPuppiupdated+process.ak4PFJetsPuppiupdatedConstituents+process.patJetsAK4PFPuppiupdatedPATJetswithUserData)
...
process.patPFMetT1SmearPuppiUpdatedpatMetUncertaintySequencePuppiUpdated = cms.Sequence(process.pfElectronsPuppiUpdated+process.pfTausPuppiUpdated+process.pfMuonsPuppiUpdated+process.pfNoPileUpPuppiUpdated+process.pfPhotonsPuppiUpdated)
...
process.fullPatMetSequenceModifiedPFMET = cms.Sequence(process.pfMetModifiedPFMET+process.pfNoPileUpJMEModifiedPFMET+process.ak4PFJetsCHSModifiedPFMET+process.patJetCorrFactorsModifiedPFMET+process.patJetsModifiedPFMET+process.basicJetsForMetModifiedPFMET+process.jetSelectorForMetModifiedPFMET+process.cleanedPatJetsModifiedPFMET+process.metrawCaloModifiedPFMET+process.pfCHS+process.pfMetCHS+process.patCHSMet+process.pfTrk+process.pfMetTrk+process.patTrkMet+process.patPFMetModifiedPFMET+process.patMetCorrectionSequenceModifiedPFMET+process.patMetUncertaintySequenceModifiedPFMET+process.patShiftedModuleSequenceModifiedPFMET+process.patCaloMet+process.slimmedMETsModifiedPFMET)
...
process.p = cms.Path(process.triggerSelection+process.recalib_sequence+process.puppiMETSequence+process.fullPatMetSequencePuppiUpdated+process.fullPatMetSequenceModifiedPFMET+process.egammaPostRecoSeq+process.rerunMvaIsolationSequence+process.slimmedTausNewID+process.ecalBadCalibReducedMINIAODFilter+process.ggNtuplizer)
