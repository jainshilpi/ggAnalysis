...
const EcalRecHit * ggNtuplizer::getECALrecHit(const DetId & id, noZS::EcalClusterLazyTools & ltNoZS, int di, int dj) {
  ...
  const EcalRecHitCollection *recHits = (nid.subdetId() == EcalBarrel) ? ltNoZS.getEcalEBRecHitCollection() : ltNoZS.getEcalEERecHitCollection();
  EcalRecHitCollection::const_iterator idRecHitIterator = recHits->find(nid);
...
Float_t ggNtuplizer::ECALrecHitE(const DetId & id, noZS::EcalClusterLazyTools & ltNoZS, int di, int dj) {
  ...
  return ((nidRH) ? nidRH->energy() : -9999.);
};
...
Float_t ggNtuplizer::etaWing(const DetId & id, noZS::EcalClusterLazyTools & ltNoZS) {
  ...
  const EcalRecHit * rh0 = getECALrecHit(id, ltNoZS, 0, 0);
  if (!rh0) return -9999;

  const EcalRecHit * rhL = getECALrecHit(id, ltNoZS, -1,  0);
  if (!rhL) return -9999;

  const EcalRecHit * rhR = getECALrecHit(id, ltNoZS, 1,  0);
  if (!rhR) return -9999;

  return (std::max(rhL->energy(), rhR->energy()) / rh0->energy());
};
...
Float_t ggNtuplizer::swissCross(const DetId & id, noZS::EcalClusterLazyTools & ltNoZS) {
  ...
  const EcalRecHit * rhU = getECALrecHit(id, ltNoZS, 0,  1);
  if (!rhU) return -9999;

  const EcalRecHit * rhD = getECALrecHit(id, ltNoZS, 0,  -1);
  if (!rhD) return -9999;

  Float_t s4 = rhL->energy() + rhR->energy() + rhU->energy() + rhD->energy();
  return (1 - s4 / rh0->energy());
};
